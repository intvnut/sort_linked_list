// Benchmarks linked lists of cacheline-sized entities.
//
...
// the specified seed.
static CachelineListNode *generate_cacheline_list(
    CachelineListNode *const nbuf, 
    const size_t elems,
    const uint64_t seed
) {
...
static uint64_t check_cacheline_list_correctness(
    CachelineListNode *const head,
    const size_t elems
) {
  uint64_t csum = 0;
  ...
  for (size_t i = 0; i < elems; ++i) {
    ...
    node = (CachelineListNode *)node->node.next;
  }
...
static TestResult run_cacheline_list_test(void *const buf, const size_t elems,
  ...
  const TestResult test_result = {
      .time = t2 - t1,
      .csum = check_cacheline_list_correctness((CachelineListNode *)out, elems)
  };
...
int main() {
  void *buf = memalign(sizeof(CachelineListNode), MAX_BYTES);

  // Warmup.
  {
    const size_t elems = MAX_BYTES / sizeof(CachelineListNode);
  ...
  for (int pow2 = 4; pow2 <= MAX_POW2; pow2++) {
    ...
    for (int sub_pow2 = 0; sub_pow2 < 8; sub_pow2++) {
      ...
      const size_t elems = size / sizeof(CachelineListNode);
      ...
      for (int seed = 1; seed <= NUM_SEEDS; ++seed) {
        const TestResult bui1_tr =
          run_cacheline_list_test(buf, elems, bui1_merge_sort, seed);
        const TestResult bui2_tr =
          run_cacheline_list_test(buf, elems, bui2_merge_sort, seed);
        ...
        const TestResult tdi2_tr =
          run_cacheline_list_test(buf, elems, tdi2_merge_sort, seed);
